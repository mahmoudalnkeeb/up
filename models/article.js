...
class Article {
  ...
  async getArticles(created_at, user_id) {
    ...
    let res1 = await con.query(query1, [user_id]);
  ...
  async userArticles(user_id) {
    ...
    let con = await db.connect();
    let res = await con.query(query, [user_id]);
    let userArticles = res.rows;
    return userArticles;
  }
  async newArticle(title, content, photo, user_id) {
    let query =
      'INSERT INTO articles(title , content ,  photo , user_id) VALUES($1 ,$2 ,$3 , $4) RETURNING *';
    let con = await db.connect();
    let res = await con.query(query, [title, content, photo, user_id]);
    let data = res.rows[0];
    return data;
  }
  async upArticle(id) {
  ...
  async editArticle(id, content, title) {
    ...
    let con = await db.connect();
    let res = await con.query(query, [content, title, id]);
    let article = res.rows[0];
    return article;
  }
