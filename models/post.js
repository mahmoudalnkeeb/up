...
class Post {
  ...
  async getFollowingPosts(id, created_at) {
    ...
    let res1 = await pool.query(query1, [id]);
  ...
  async createPost(userId, content, photo) {
    ...
    let res = await pool.query(query, [content, photo, userId]);
    let data = res.rows[0];
    return data;
  }

  async upPost(id) {
    let query = 'UPDATE posts SET ups = ups + 1 WHERE id = $1';
    let res = await pool.query(query, [id]);
    let post = res.rows[0];
    return post;
  }
  async editPost(id, content) {
    ...
    let res = await pool.query(query, [content, id]);
    let post = res.rows[0];
    return post;
  }
  async deletePost(id) {
