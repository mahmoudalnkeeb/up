...
class User {
  ...
  async login(username) {
    ...
    let res = await pool.query(query, [username]);
    let data = res.rows[0];
  ...
  async follow(id, follow_id) {
    ...
    let res1 = await pool.query(query1, [follow_id, id]);
    let res2 = await pool.query(query2, [id, follow_id]);
  ...
  async unfollow(id, unfollow_id) {
    ...
    let res1 = await pool.query(query1, [unfollow_id, id]);
    let res2 = await pool.query(query2, [id, unfollow_id]);
    return { data1: res1.rows, data2: res2.rows };
  }
  async updateName(fname, lname, id) {
    ...
    let res = await pool.query(query, [fname, lname, id]);
    let data = res.rows[0];
  ...
  async updatePass(id, newPass) {
    ...
    let res = await pool.query(query, [newPass, id]);
    let data = res.rows[0];
